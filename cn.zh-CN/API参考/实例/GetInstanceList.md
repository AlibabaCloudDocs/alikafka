...
|SecurityGroup|String|sg-bp13wfx7kz9pkowc\*\*\*|实例的安全组。
 ...
 -   如果实例是通过消息队列Kafka版控制台或调用StartInstance（调用时没有配置安全组）部署的，则返回值为空。
-   如果实例是通过调用StartInstance（调用时配置了安全组）部署的，则返回值为配置的安全组。 |
...
|UpgradeServiceDetailInfo|Array| |实例的升级信息。 |
